...
public class OrderServiceImpl implements OrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        //将已选中的商品转为订单明细
        ServerResponse serverResponse=getCatrOrderItem(userId,cartList);
        if (!serverResponse.isSuccess()){
            return serverResponse;
        }
        //创建订单并保存到数据库
        //计算订单的总价格new BigDecimal()
        ...
        orderItemMapper.inserBatch(orderItemList);
        //扣除商品库存
    ...
    private OrderVo assembleOrderVO(Order order,List<OrderItem> orderItemList,Integer shippingId){
        ...
        if (shipping!=null){
            ...
            orderVo.setShoppingVo(shippingVo);
            orderVo.setReceiverName(shipping.getReceiverName());
            orderVo.setReceiverName(shipping.getReceiverName());
        }
        orderVo.setStatus(order.getStatus());
        ResponseCodeCategory responseCodeCategory=ResponseCodeCategory.orderpaystatus(order.getStatus());
        if (responseCodeCategory!=null){
            orderVo.setStatusDesc(responseCodeCategory.getMsg());
        }
        orderVo.setPosetage(0);
        orderVo.setPayment(order.getPayment());
        orderVo.setPaymentType(order.getPaymentType());
        ResponseCodeCategory responseCodeCategory1=ResponseCodeCategory.orderpaystatus(order.getPaymentType());
        if (responseCodeCategory1!=null) {
            orderVo.setPaymentTypeDesc(responseCodeCategory1.getMsg());
        }
    ...
    private ShippingVo assmbleShippingVo(Shipping  shipping){
        ...
        if (shippingVo!=null){
            ...
            shippingVo.setReceiverPhoto(shipping.getReceiverPhoto());
    ...
    private OrderItemVo assembleOrderItemVO(OrderItem orderItem){
        ...
        if (orderItem!=null){
            ...
            orderItemVo.setCurrentUtinPrice(orderItem.getProductUnitPrice());
...
private  void  reduceProductStock(List<OrderItem> orderItemList){
    ...
    private Order createOrder(Integer userId, Integer shippingId, BigDecimal orderTotalPrice){
        ...
        order.setOrderNo(generataeOrderNo());
        order.setUserId(userId);
        order.setShippingId(shippingId);
        order.setStatus(ResponseCodeCategory.ORDER_UN_PAY.getStatus());
        order.setPayment(orderTotalPrice);
        order.setPostage(0);
        order.setPaymentType(ResponseCodeCategory.ONLINE.getStatus());
        //保存订单
     ...
     * 生成订单编号
     */
    private  Long generataeOrderNo(){
        //得到当前的一个时间戳   生成一个随机数    生成订单号  适用于用户不多的情况下
     ...
     * @return
     */
    private ServerResponse getCatrOrderItem(Integer userId,List<Cart> cartList){
        //参数不能为空
        if (cartList==null||cartList.size()==0){
            return ServerResponse.createServerResponseByError("购物车是空的");
        }
        ...
        for (Cart cart:cartList){
           ...
           orderItem.setUserId(userId);
            ...
            if (product.getStatus()!=ResponseCodeCategory.PRODUCT_ONLINE.getStatus()){//商品下架
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        //判断订单状态并取消
        if (order.getStatus()!=ResponseCodeCategory.ORDER_UN_PAY.getStatus()){
            return ServerResponse.createServerResponseByError("订单不可取消");
        }
        //返回结果
        order.setStatus(ResponseCodeCategory.ORDER_CANCELED.getStatus());
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        cartOrderItemVo.setImageHost(PropertiesUtil.readByKey("imageHost"));
        List<OrderItem> orderItemList=(List<OrderItem>) serverResponse.getData();
        List<OrderItemVo> orderItemVoList=Lists.newArrayList();
        if (orderItemList==null||orderItemList.size()==0){
            return ServerResponse.createServerResponseByError("购物车为空");
    ...
    public ServerResponse list(Integer userId,Integer pageNum,Integer pageSize) {
        ...
        if (orderList==null||orderList.size()==0){
            return  ServerResponse.createServerResponseByError("为查询到订单");
        }
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
       ...
       Order order= orderMapper.findOrderByOrderNo(orderNo);
        ...
        try {
            ...
            return ServerResponse.createServerResponseBySuccess(serverResponse);
        }catch (Exception e){
            e.printStackTrace();
        }

        return ServerResponse.createServerResponseByError("支付失败");
    }
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        if (order.getStatus()>=ResponseCodeCategory.ORDER_PAYED.getStatus()){
            //防止支付宝重复回调
        ...
        if (trade_status.equals(Const.TRADE_SUCCESS)){
            //支付成功
            //更改订单状态，更改支付时间
            order.setStatus(ResponseCodeCategory.ORDER_PAYED.getStatus());
        ...
        payInfo.setPayPlatform(ResponseCodeCategory.AlIPAY.getStatus());
        payInfo.setPlatformStatus(trade_status);
        payInfo.setPlatfromNumber(tarde_no);
        payInfo.setUserId(order.getUserId()) ;
        int result=payInfoMapper.insert(payInfo);
        if (result>0){
            return ServerResponse.createServerResponseBySuccess();
        }
        return ServerResponse.createServerResponseByError("支付i信息导出失败");
    }
     ...
     */
        ...
        public ServerResponse pay(Order order) {
            ...
            //订单标题
            String subject = "订单" + order.getOrderNo() + "当面付扫码消费" + order.getPaymentType().intValue();
            ...
            List<OrderItem> orderItemList = orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
            //将orderItemList转换为goodsDetailList 使用for循环遍历
            if (orderItemList != null && orderItemList.size() > 0) {
                for (OrderItem orderItem : orderItemList) {
                    GoodsDetail goodsDetail = GoodsDetail.newInstance(String.valueOf(orderItem.getProductId()), orderItem.getProductName(), orderItem.getProductUnitPrice().longValue(), orderItem.getQuantity());
...
*/
            ...
            AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                    ...
                    .setNotifyUrl("http://9e9p6r.natappfree.cc/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
            ...
            switch (result.getTradeStatus()) {
                case SUCCESS:
                    ...
                    // 需要修改为运行机器上的路径   %s占位符 的值为 response.getOutTradeNo()订单号
                    String filePath = String.format("D:/Java笔记/面试攻关/onload/qr-%s.png",
                            response.getOutTradeNo());
                    log.info("filePath:" + filePath);
                    //将二维码生成，将生成的图片写入到文件目录下
                    ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                   /* File file = new File(filePath);
                    FTPUtil.uploadFile(Lists.newArrayList(file));*/
                    Map map = Maps.newHashMap();
                    map.put("orderNo", order.getOrderNo());
                    map.put("qrCode", PropertiesUtil.readByKey("imageHost" )+ "/qr-" + response.getOutTradeNo() + ".png");
                ...
                return ServerResponse.createServerResponseByError("交易失败");
        }
    }
